...
import { ArrowLeft, ClipboardList, FlaskConical, Pill, Stethoscope, Activity, Plus, Loader2, FileText, ExternalLink, Printer, UploadCloud, Star } from "lucide-react";
    ...
    <div className="mx-auto max-w-6xl space-y-4 p-6">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          <FavoriteToggle patientId={patient.id} userId={user.id} />
...
function FavoriteToggle({ patientId, userId }: { patientId: string; userId: string }) {
  const qc = useQueryClient();
  const { data: isFavorite } = useQuery({
    queryKey: ["favorite", userId, patientId],
    queryFn: async () => {
      const { data } = await supabase
  ...
  const toggle = useMutation({
    mutationFn: async () => {
      if (isFavorite) {
        const { error } = await supabase.from("patient_favorites").delete().eq("user_id", userId).eq("patient_id", patientId);
        if (error) throw error;
      } else {
        const { error } = await supabase.from("patient_favorites").insert({ user_id: userId, patient_id: patientId });
        if (error) throw error;
      }
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["favorite", userId, patientId] });
  ...
  return (
    ...
    >
      <Star className={`h-4 w-4 ${isFavorite ? "fill-current text-amber-500" : "text-muted-foreground"}`} />
    </Button>
  );
}
  ...
  async function print() {
    ...
    try {
      const [clinicRes, docRes] = await Promise.all([
        supabase.from("clinics").select("name, address, phone, email, logo_url").eq("id", patient.clinic_id).maybeSingle(),
