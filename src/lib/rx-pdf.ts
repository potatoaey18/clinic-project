...
/** Loads a (possibly cross-origin) image and re-encodes it as a PNG data URL via canvas, so jsPDF can embed any source format (including WebP). Returns null on any failure — logo is a nice-to-have, never worth breaking the PDF over. */
...
export async function generatePrescriptionPdf(input: {
  clinic: { name?: string | null; address?: string | null; phone?: string | null; email?: string | null; logo_url?: string | null };
  ...
  const logoDataUrl = input.clinic.logo_url ? await loadImageAsPngDataUrl(input.clinic.logo_url) : null;
  if (logoDataUrl) {
    try {
      doc.addImage(logoDataUrl, "PNG", 30, 24, 32, 32);
    } catch {
      // Corrupt/unsupported image — skip silently, the rest of the Rx still prints fine.
    }
  }
