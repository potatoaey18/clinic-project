...
import { displayName, avatarInitials } from "@/lib/profile";
...
function AuthedLayout() {
  ...
  const { mode, setMode, registerUser } = useTheme();

  useEffect(() => {
    registerUser(user.id);
    return () => registerUser(undefined);
          ...
          <header className="sticky top-0 z-10 flex h-14 items-center gap-3 border-b border-border bg-background/80 px-4 backdrop-blur">
            ...
            <div className="ml-auto flex items-center gap-2">
              <Button
                ...
                size="icon"
                className="h-8 w-8"
                title={mode === "system" ? `Following system (${isDark ? "dark" : "light"})` : mode === "dark" ? "Dark mode" : "Light mode"}
